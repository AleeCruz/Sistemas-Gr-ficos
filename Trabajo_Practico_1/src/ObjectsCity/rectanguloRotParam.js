...
/**
 * Crea un rectángulo que se barre hacia arriba rotando, usando geometría paramétrica.
...
export function crearRectanguloBarridoGirandoParametrico(
  ...
  // 🧠 Función paramétrica (u: altura, v: contorno del rectángulo)
  const superficieParametrica = (u, v, target) => {
    const y = u * altura - 0.35;
    const angle = u * rotacionTotalRad;

    let x = 0, z = 0;
    const p = v * 2 * (ladoX + ladoZ); // Normalizamos 'v' para recorrer el perímetro
